var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
import LocalForage from 'jeep-localforage';
import { Data } from "./Data";
//const DATABASE: string = "storageIDB";
//const STORAGESTORE: string = "storage_store";
var StorageDatabaseHelper = /** @class */ (function () {
    function StorageDatabaseHelper(dbName, tableName) {
        this._db = null;
        this.openStore(dbName, tableName);
    }
    StorageDatabaseHelper.prototype.openStore = function (dbName, tableName) {
        var ret = false;
        var config = this.setConfig(dbName, tableName);
        this._db = LocalForage.createInstance(config);
        if (this._db != null) {
            this._dbName = dbName;
            ret = true;
        }
        return ret;
    };
    StorageDatabaseHelper.prototype.setConfig = function (dbName, tableName) {
        var config = {
            name: dbName,
            storeName: tableName,
            driver: [LocalForage.INDEXEDDB, LocalForage.WEBSQL],
            version: 1
        };
        return config;
    };
    StorageDatabaseHelper.prototype.setTable = function (tableName) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                return [2 /*return*/, this.openStore(this._dbName, tableName)];
            });
        });
    };
    StorageDatabaseHelper.prototype.set = function (data) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                return [2 /*return*/, this._db.setItem(data.name, data.value).then(function () {
                        return Promise.resolve(true);
                    })
                        .catch(function (error) {
                        console.log('set: Error Data insertion failed: ' + error);
                        return Promise.resolve(false);
                    })];
            });
        });
    };
    StorageDatabaseHelper.prototype.get = function (name) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                return [2 /*return*/, this._db.getItem(name).then(function (value) {
                        var data = new Data();
                        data.name = name;
                        data.value = value;
                        return Promise.resolve(data);
                    })
                        .catch(function (error) {
                        console.log('get: Error Data retrieve failed: ' + error);
                        return Promise.resolve(null);
                    })];
            });
        });
    };
    StorageDatabaseHelper.prototype.remove = function (name) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                return [2 /*return*/, this._db.removeItem(name).then(function () {
                        return Promise.resolve(true);
                    })
                        .catch(function (error) {
                        console.log('remove: Error Data remove failed: ' + error);
                        return Promise.resolve(false);
                    })];
            });
        });
    };
    StorageDatabaseHelper.prototype.clear = function () {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                return [2 /*return*/, this._db.clear().then(function () {
                        return Promise.resolve(true);
                    })
                        .catch(function (error) {
                        console.log('clear: Error Data clear failed: ' + error);
                        return Promise.resolve(false);
                    })];
            });
        });
    };
    StorageDatabaseHelper.prototype.keys = function () {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                return [2 /*return*/, this._db.keys().then(function (keys) {
                        return Promise.resolve(keys);
                    })
                        .catch(function (error) {
                        console.log('keys: Error Data retrieve keys failed: ' + error);
                        return Promise.resolve(null);
                    })];
            });
        });
    };
    StorageDatabaseHelper.prototype.values = function () {
        return __awaiter(this, void 0, void 0, function () {
            var values;
            return __generator(this, function (_a) {
                values = [];
                return [2 /*return*/, this._db.iterate((function (value) {
                        values.push(value);
                    })).then(function () {
                        return Promise.resolve(values);
                    })
                        .catch(function (error) {
                        console.log('values: Error Data retrieve values failed: ' + error);
                        return Promise.resolve(null);
                    })];
            });
        });
    };
    StorageDatabaseHelper.prototype.keysvalues = function () {
        return __awaiter(this, void 0, void 0, function () {
            var keysvalues;
            return __generator(this, function (_a) {
                keysvalues = [];
                return [2 /*return*/, this._db.iterate((function (value, key) {
                        var data = new Data();
                        data.name = key;
                        data.value = value;
                        keysvalues.push(data);
                    })).then(function () {
                        return Promise.resolve(keysvalues);
                    })
                        .catch(function (error) {
                        console.log('keysvalues: Error Data retrieve keys/values failed: ' + error);
                        return Promise.resolve(null);
                    })];
            });
        });
    };
    StorageDatabaseHelper.prototype.iskey = function (name) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                return [2 /*return*/, this.get(name).then(function (data) {
                        if (data.value != null) {
                            return Promise.resolve(true);
                        }
                        else {
                            return Promise.resolve(false);
                        }
                    })
                        .catch(function (error) {
                        console.log('iskey: Error Data retrieve iskey failed: ' + error);
                        return Promise.resolve(false);
                    })];
            });
        });
    };
    return StorageDatabaseHelper;
}());
export { StorageDatabaseHelper };
//# sourceMappingURL=StorageDatabaseHelper.js.map