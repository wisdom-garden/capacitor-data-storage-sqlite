...
public class StorageDatabaseHelper {
    ...
    private void InitializeSQLCipher(Context context) {
        ...
        if(!encrypted && mode.equals("no-encryption")) {
            ...
            try {
                database = SQLiteDatabase.openOrCreateDatabase(databaseFile.getAbsolutePath(), "", null, null, null);
                isOpen = true;
            } catch (Exception e) {
                database = null;
            }
            ...
            createFilePath(databaseFile);
            try {
                database = SQLiteDatabase.openOrCreateDatabase(databaseFile.getAbsolutePath(), secret, null, null, null);
                isOpen = true;
            } catch (Exception e) {
            ...
            try {
                ...
                Log.d(TAG, "InitializeSQLCipher: Failed to open encrypted database: " + e);
                database = null;
            }

            if (database != null && tempFile != null && tempFile.exists()) {
                SQLiteDatabase tempDB = null;
                try {
                    tempDB = SQLiteDatabase.openOrCreateDatabase(tempFile.getAbsolutePath(), "", null, null, null);
                    ...
                    Log.d(TAG, "InitializeSQLCipher: Error during encryption migration: " + e);
                    isOpen = false;
                } finally {
    ...
    private SQLiteDatabase getDB() {
        if (_db == null || !_db.isOpen()) {
            String path = appContext.getDatabasePath(dbName).getAbsolutePath();
            _db = SQLiteDatabase.openOrCreateDatabase(path, secret, null, null, null);
            int version = _db.getVersion();
            if (version == 0) {
                onCreate(_db);
                _db.setVersion(dbVersion);
            } else if (version != dbVersion) {
                onUpgrade(_db, version, dbVersion);
                _db.setVersion(dbVersion);
            }
        }
        return _db;
    ...
    // delete all data into the database
